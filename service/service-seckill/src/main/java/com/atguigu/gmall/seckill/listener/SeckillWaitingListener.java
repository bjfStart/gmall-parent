...
public class SeckillWaitingListener {
    ...
    public void seckillWaiting(Message message, Channel channel) throws IOException {
        ...
        try {
            ...
            //库存扣减成功
            rabbitTemplate.convertAndSend(MqConst.ORDER_EVENT_EXCHANGE + msg.getSkuCode());
            //reids修改一下标志位
            String json = redisTemplate.opsForValue().get(SysRedisConst.SECKILL_ORDERD + msg.getSkuCode());
            OrderInfo orderInfo = Jsons.toObj(json, OrderInfo.class);
            //代表库存扣减操作过了
            orderInfo.setOperateTime(new Date());
            redisTemplate.opsForValue().set(SysRedisConst.SECKILL_ORDERD + msg.getSkuCode(), Jsons.toStr(orderInfo));
            ...
            redisTemplate.opsForValue().set(SysRedisConst.SECKILL_ORDERD + msg.getSkuCode(),"x");
            channel.basicAck(tag,false);
        } catch (Exception e) {
            log.error("业务失败：{}",e);
            rabbitService.retryConsumeMsg(10L,SysRedisConst.MQ_RETRY+msg.getSkuCode(),tag,channel);
            e.printStackTrace();
        }
