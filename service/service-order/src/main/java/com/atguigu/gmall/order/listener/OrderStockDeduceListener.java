...
public class OrderStockDeduceListener {
    ...
    @RabbitListener(bindings = {
            @QueueBinding(
                    value = @Queue(name = MqConst.ORDER_WARE_QUEUE,
                            durable = "true",exclusive = "false",autoDelete = "false"),
                    exchange = @Exchange(name = MqConst.ORDER_WARE_EXCHANGE,
    ...
    public void stockDeduceListener(Message message, Channel channel) throws IOException {
        ...
        try {
            ...
            //1.改造业务：每次发消息的时候都带上
            //2.Sharding：不带分片键，查询可以（查询所有库所有表）、增删改不行
            ...
            switch (msg.getStatus()){
                ...
                case "OUT_OF_STOCK" : status = ProcessStatus.STOCK_OVER_EXEPTION;break;
            ...
            rabbitService.retryConsumeMsg(10L,uk,tag,channel);
        }
    }
}
