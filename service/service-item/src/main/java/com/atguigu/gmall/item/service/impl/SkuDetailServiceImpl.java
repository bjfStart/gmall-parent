...
public class SkuDetailServiceImpl implements SkuDetailService {
    ...
    public void updateHotScore(long skuId) {
        //redis统计得分
        Long increment = redisTemplate.opsForValue().increment(SysRedisConst.SKU_HOTSCORE_PREFX + skuId);
        if(increment % 100 == 0){
            //累计到一定量更新es
...
//       return skuDetailToResult.getData();
    ...
    public SkuDetailTo getSkuDetailWithCache(long skuId) {
        ...
        if (cacheData == null) {
            ...
            //6.布隆说有，可能有，就需要回源查数据
            boolean lock = cacheOpsService.tryLock(skuId);//为当前商品加自己的分布式锁，100w的49插叙只会放进一个
            if (lock) {
                //7.获取锁成功，查询远程
                log.info("[{}]商品 缓存未命中给，布隆说有，准备回源。。。。" + skuId);
...
//    }
    ...
    public SkuDetailTo getSkuDetailXxxxFeature(long skuId) {
        ...
        if (StringUtils.isEmpty(jsonStr)) {
            ...
            // 防止随机值穿透攻击？回源之前，先用布隆/bitmap判断有没有
//            int result = getbit(49);
            //TODO 加锁解决击穿
            SkuDetailTo fromRpc = null;
            //JVM 抢不到锁的等1s，怎么判断 synchronized 抢到还是没抢到

//            ReentrantLock lock = new ReentrantLock(); //锁不住
//            lock.lock(); //等锁，必须等到锁

            //判断锁池中是否有自己的锁
            //锁池中不存在就放一把新的锁，作为自己的锁，存在就用之前锁
